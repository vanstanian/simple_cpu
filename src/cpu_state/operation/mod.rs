...
impl OperLogic for Operation {
    ...
    fn to_string(self) -> String {
        match self {
            ...
            Operation::Jmp {
                reg_cond,
                pc_to_jump,
            } => format!("jmp {} {}", reg_cond, pc_to_jump),
        }
    }
    ...
    fn compute(self, cpu_state: CpuState) -> CpuState {
        match self {
            Operation::Add { op1, op2, op3 } => cpu_state.clone().alloc(
                private::get_mem_val(op1, &cpu_state),
                private::get_val(op2, &cpu_state) + private::get_val(op3, &cpu_state),
            ),
            Operation::Sub { op1, op2, op3 } => cpu_state.clone().alloc(
                private::get_mem_val(op1, &cpu_state),
                private::get_val(op2, &cpu_state) - private::get_val(op3, &cpu_state),
            ),
            Operation::Mul { op1, op2, op3 } => cpu_state.clone().alloc(
                private::get_mem_val(op1, &cpu_state),
                private::get_val(op2, &cpu_state) * private::get_val(op3, &cpu_state),
            ),
            Operation::Div { op1, op2, op3 } => cpu_state.clone().alloc(
                private::get_mem_val(op1, &cpu_state),
                private::get_val(op2, &cpu_state) / private::get_val(op3, &cpu_state),
            ),
            Operation::Ld { dir, at } => cpu_state.clone().move_to_cpu_memory(
                private::get_mem_val(dir, &cpu_state),
                at.parse::<usize>().unwrap(),
            ),
            Operation::Str { reg, to } => cpu_state.clone().move_to_main_memory(
                private::get_mem_val(reg, &cpu_state),
                to.parse::<usize>().unwrap(),
            ),
            Operation::Mov { from_reg, to_reg } => cpu_state.clone().move_on_cpu_memory(
            ...
            Operation::Cmp { this, eq_this } => {
                ...
                cpu_state
                    .clone()
                    .alloc(private::get_mem_val(this, &cpu_state), cmp)
            }
            Operation::Jmp {
                reg_cond,
                pc_to_jump,
